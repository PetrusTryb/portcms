...
export default function Property(property: PropertyProps<PropertyTypes>) {
    switch (property.type) {
        case "string":
            return <input type="text" className="form-input w-full" defaultValue={property.value as string}
                          onChange={(e) => property.onChange && property.onChange(e.target.value)}/>;
        case "number":
            return <input type="number" className="form-input w-full" value={property.value as number}
